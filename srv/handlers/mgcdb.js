...
module.exports = cds.service.impl(async function () {
    ...
    this.on("GET", "TimeSheetDetails", async (req, next) => {
        return next();
    });


    this.on("UPDATE", "TimeSheetDetails", async (req, next) => {

        const {
            TimeSheetDetails, Employees, Job, Section, Phase
        } = cds.entities('mgc.srv.MgcDbService');
        const srv = cds.services['mgc.srv.MgcDbService'];

        let entity = cds.entities('mgc.srv.MgcDbService')['TimeSheetDetails'];
        let cdsEntity = req.target.projection.from.ref[0];
        ...
        if (employeeId) {
            ...
            if (req.data.WorkOrder) {
                var empWorkHours = await SELECT.from(Employees).where(whereObject);
            ...
            if (req.data.CostCenter) {
                var empWorkHours2 = await SELECT.from(Employees).where(whereObject);
            ...
            if (req.data.ReceivingUnitTruck) {
                var empWorkHours1 = await SELECT.from(Employees).where(whereObject);
        ...
        if (req.data.AppName === "CP_CREW" && employeeId === "") {
            if (req.data.WorkOrder) {
                ...
                var empWorkHours = await SELECT.from(Employees).where(whereObjectCrew);
                if (empWorkHours.length > 0) {
                    ManagerApprovalEmail1 = empWorkHours[0].PositionManagerEmail == null ? '' : empWorkHours[0].PositionManagerEmail;
                    ManagerApprovalName = empWorkHours[0].PositionManagerName == null ? '' : empWorkHours[0].PositionManagerName;
                }
        ...
        if (req.data.AppName === "DD") {
            if (req.data.SendingUnitTrailer) {
               if (req.data.CostCenter) {
                   let whereObjectDD = { ID: req.data.ForemanID };
                   var empWorkHours = await SELECT.from(Employees).where(whereObjectDD);
                   if (empWorkHours.length > 0) {
                       ManagerApprovalEmail1 = empWorkHours[0].PositionManagerEmail == null ? '' : empWorkHours[0].PositionManagerEmail;
                       ManagerApprovalName = empWorkHours[0].PositionManagerName == null ? '' : empWorkHours[0].PositionManagerName;
                   }
        ...
        if (req.data.Phase) {
            var phaseWorkHours = await SELECT.from(Phase).where(whereObject1);
        ...
        if (req.data.Section) {
            if (ManagerApprovalEmail1 == '') {
                var SectionWorkHours = await SELECT.from(Section).where(whereObject2);
        ...
        if (req.data.Job) {
            if (ManagerApprovalEmail1 == '') {
                var jobWorkHours = await SELECT.from(Job).where(whereObject3);
    ...
    this.on("POST", "TimeSheetDetails", async (req, next) => {
        let cdsEntity = req.target.projection.from.ref[0];
        let id = req.data.ID;
        const {
            TimeSheetDetails, Employees, Job, Section, Phase
        ...
        if (req.data.Phase) {
            ...
            if (phaseWorkHours.length > 0) {
                ...
                // ManagerApprovalEmail1 = phaseWorkHours[0].ProjectManagerEmail == null ? '' : phaseWorkHours[0].ProjectManagerEmail;
            }
        }
        if (req.data.Section) {
            if (ManagerApprovalEmail1 == '') {
    ...
    this.on('RTOTCalulation', async (req) => {
        ...
        const srv = cds.services['mgc.srv.MgcDbService'];
        let cdsEntity = 'mgc.db.TimeSheetDetails';
        ...
        if (req.data.OtFrequency === "Weekly") {
            ...
            try {
                ...
                var empWorkHours = await SELECT.from(TimeSheetDetails).where(whereObject);
            } catch (error) {
